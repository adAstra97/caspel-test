...
const Home = () => {
  return (
    <Container>
      <h1>Manage Data</h1>
      <div className="py-3 px-4 border-2 rounded-2xl border-border shadow-sm shadow-border overflow-x-auto">
        <div>Control Panel</div>
        <table className="w-full">
          <caption className="mb-3 text-size-20 text-right">Users 2025</caption>
          ...
          <tbody>
            {MOCKED_DATA.map(({ id, name, date, value }) => (
              <tr
                key={id}
                className="hover:bg-border/5 hover:transition-colors duration-100"
              >
